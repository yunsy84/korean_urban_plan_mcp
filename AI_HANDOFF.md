...
> 목적: 새 대화방에서 `korean_urban_plan_mcp` 작업을 이어갈 때, 이전 대화의 장황한 요약 대신 **이 문서 + 실제 GitHub 코드**를 현재 상태의 기준으로 사용한다.
>
> 최우선 원칙: 문서와 실제 코드가 서로 다르면 **실제 코드와 최신 커밋을 우선 확인**하고, 문서의 오류를 발견하면 작업 과정에서 정정한다.
...
`68afe5184c943d2920a9bac5e9d7fa649ebc9ff4`

커밋 메시지:

`Update tests for current urban plan architecture`
...
현재 확인 당시 실제 프로젝트 폴더의 로컬 체크아웃 브랜치는 `main`이고,
원격에는 `origin/urban-plan-source-evidence`가 존재했다.

따라서 **새 대화에서 바로 Pull / Reset / 강제 Switch를 임의로 실행하지 않는다.**
먼저 현재 로컬 working tree와 HEAD를 확인한다.
...
따라서 새 대화에서 작업을 재개할 때는:

**1순위: 실제 브랜치의 코드**
  
**2순위: AI_HANDOFF.md**
  
**3순위: README.md / CHANGELOG**

순으로 본다.

---

## 14. 새 대화방에서 작업을 시작하는 방법

새 대화 첫 메시지에서 다음 취지로 지시한다.

```
GitHub yunsy84/test_file의 urban-plan-source-evidence 브랜치를 현재 작업 기준으로 사용한다.

먼저 AI_HANDOFF.md를 읽고,
그 다음 실제 브랜치의 관련 코드를 확인한 뒤
현재 상태를 기준으로 작업을 이어가라.

이전 대화 내용을 추측해서 재구성하지 말고,
실제 코드와 AI_HANDOFF.md를 기준으로 판단하라.

코드 수정 전에는 현재 파일 원문과 실제 실행 상태를 확인하고,
검증되지 않은 내용은 성공했다고 말하지 말라.
```
...
이 문서는 **기억을 대신하는 단일 인계 기준 문서**로 유지한다.
