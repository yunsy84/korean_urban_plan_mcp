...
## 33. 결정도·지형도면 전체 페이지 이미지 보존

`saveMatchedImages=true`일 때 OCR marker가 있는 페이지뿐 아니라 `sourceMaterials`가 식별한 전체 결정도·지형도면 범위도 이미지로 보존한다.

실제 천안시 고시 제2020-2호 PDF에서는:
...
이 기능은 해당 필지가 해당 도면에 공간적으로 포함된다고 자동 판정하지 않는다. 전체 관련 페이지의 원본 렌더링 이미지를 보존하여 후속 시각 검토와 공간분석에 사용할 수 있게 하는 단계다.


---

## 34. 2026-09-26 공간자료 페이지 보존 확인 및 다음 방향

실제 `analyzeUrbanPlan()` 호출에서 다음 원자료 페이지 이미지가 생성되었다.
...
중간 페이지도 전체 범위 보존되며 각 `ocrPages[].imagePath`에서 추적 가능하다.

다만 이는 필지의 공간적 포함 여부를 자동 확정하는 기능이 아니다.
